...
import com.example.mystudyzone.WeekViewEvent;
...
/**
 * Created by ci-and-10 on 03/05/17.
 */
...
public class WeekView extends View {
    ...
    private Paint mHeaderDayTextPaint;
    ...
    private Paint mTodayHeaderDayTextPaint;
    ...
    private Paint mHeaderColumnBackgroundPaintStorke;
    ...
    private int mTextTimeSize = 25;
    private int mTextDaySize = 20;
    ...
    // Listeners.
    private WeekView.HeaderViewClickListener mHeaderViewClickListener;
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            // If the tap was on in an empty space, then trigger the callback.
            if (mHeaderViewClickListener != null && e.getX() > mHeaderColumnWidth && e.getY() < (mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)) {
                Calendar selectedTime = getTimeForHeader(e.getX(), e.getY() + (mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom));
                if (selectedTime != null) {
                    playSoundEffect(SoundEffectConstants.CLICK);
                    mHeaderViewClickListener.onHeaderViewClick(selectedTime);
                }
            }
    ...
    private void init() {
        ...
        mTimeTextPaint.setTextSize(mTextTimeSize);
        ...
        mHeaderTextPaint.getTextBounds("E dd/M", 0, "E dd/M".length(), rect);
        mHeaderTextHeight = rect.height();
        mHeaderTextPaint.setTypeface(Typeface.DEFAULT_BOLD);

        // Measure settings for header Day row.
        mHeaderDayTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mHeaderDayTextPaint.setColor(mHeaderColumnTextColor);
        mHeaderDayTextPaint.setTextAlign(Paint.Align.CENTER);
        mHeaderDayTextPaint.setTextSize(mTextDaySize);
        mHeaderDayTextPaint.getTextBounds("EEE", 0, "EEE".length(), rect);
        mHeaderTextHeight = rect.height();
        mHeaderDayTextPaint.setTypeface(Typeface.DEFAULT_BOLD);
        ...
        // Prepare today header text color paint.
        mTodayHeaderDayTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mTodayHeaderDayTextPaint.setTextAlign(Paint.Align.CENTER);
        mTodayHeaderDayTextPaint.setTextSize(mTextDaySize);
        mTodayHeaderDayTextPaint.setTypeface(Typeface.DEFAULT_BOLD);
        mTodayHeaderDayTextPaint.setColor(mTodayHeaderTextColor);
        ...
        mHeaderColumnBackgroundPaint.setStyle(Paint.Style.FILL);
        mHeaderColumnBackgroundPaint.setColor(mHeaderColumnBackgroundColor);

        mHeaderColumnBackgroundPaintStorke = new Paint();
        mHeaderColumnBackgroundPaintStorke.setStyle(Paint.Style.STROKE);
        mHeaderColumnBackgroundPaintStorke.setColor(Color.parseColor("#80F17700"));
        mHeaderColumnBackgroundPaintStorke.setStrokeWidth(8.0F);
...
//        canvas.drawRect(0, mHeaderTextHeight + mHeaderRowPadding * 2, mHeaderColumnWidth, getHeight(), mHeaderColumnBackgroundPaintStorke);
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mEventRects != null) {
            for (WeekView.EventRect eventRect : mEventRects) {
        ...
        for (int dayNumber = leftDaysWithGaps + 1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            String[] splitString = dayLabel.split(" ");

            canvas.drawText(splitString[1], startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
            canvas.drawText(splitString[0], startPixel + mWidthPerDay / 2, (mHeaderTextHeight + 36) + mHeaderRowPadding, sameDay ? mTodayHeaderDayTextPaint : mHeaderDayTextPaint);
     ...
     * @return The time and date at the clicked position.
     */
    private Calendar getTimeForHeader(final float x, final float y) {
        int leftDaysWithGaps = (int) -(Math.ceil(mCurrentOrigin.x / (mWidthPerDay + mColumnGap)));
        float startPixel = mCurrentOrigin.x + (mWidthPerDay + mColumnGap) * leftDaysWithGaps +
                mHeaderColumnWidth;
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    if (left < right &&
                        ...
                        drawEventCategory(mEventRects.get(i).event, mEventRects.get(i).rectF, canvas, top, left);
                        //drawEventSubject(mEventRects.get(i).event, mEventRects.get(i).rectF, canvas, bottom, right);
                    } else
    ...
    private void drawEventSubject(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        if (rect.right - rect.left - mEventPadding * 2 < 0) return;
        if (rect.bottom - rect.top - mEventPadding * 2 < 0) return;
    ...
    private void drawEventCategory(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        if (rect.right - rect.left - mEventPadding * 2 < 0) return;
        if (rect.bottom - rect.top - mEventPadding * 2 < 0) return;
        ...
        int availableHeight = (int) (rect.bottom - originalTop - mEventPadding * 4);
        int availableWidth = (int) (rect.right - originalLeft - mEventPadding * 4);
    ...
    private void cacheEvent(WeekViewEvent event) {
        ...
        if (!isSameDay(event.getStartTime(), event.getEndTime())) {
            ...
            mEventRects.add(new WeekView.EventRect(event1, event, null));
            ...
            while (!isSameDay(otherDay, event.getEndTime())) {
                ...
                mEventRects.add(new WeekView.EventRect(eventMore, event, null));
            ...
            mEventRects.add(new WeekView.EventRect(event2, event, null));
        } else {
            mEventRects.add(new WeekView.EventRect(event, event, null));
        }
    }
    ...
    public void setOnHeaderViewClickListener(WeekView.HeaderViewClickListener listener) {
        this.mHeaderViewClickListener = listener;
    }

    public WeekView.HeaderViewClickListener getmHeaderViewClickListener() {
        return mHeaderViewClickListener;
    }
        ...
        if (mDateTimeInterpreter == null) {
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretDate(Calendar date) {
                    try {
                        SimpleDateFormat sdf = mDayNameLength == LENGTH_SHORT ? new SimpleDateFormat("dd/M", Locale.getDefault()) : new SimpleDateFormat("dd/M", Locale.getDefault());
                        SimpleDateFormat sdf1 = mDayNameLength == LENGTH_SHORT ? new SimpleDateFormat("EEE", Locale.getDefault()) : new SimpleDateFormat("EEE", Locale.getDefault());
                        String dayString = sdf1.format(date.getTime()).toUpperCase();
                        return dayString + " " + sdf.format(date.getTime()).toUpperCase();
                    } catch (Exception e) {
                        e.printStackTrace();
                        return "";
                    }
                ...
                public String interpretTime(int hour) {
                    ...
                    try {
                        ...
                        SimpleDateFormat sdf1 = DateFormat.is24HourFormat(getContext()) ? new SimpleDateFormat("HH:mm", Locale.getDefault()) : new SimpleDateFormat("a", Locale.getDefault());
                        if (DateFormat.is24HourFormat(getContext())) {
                            return sdf.format(calendar.getTime());
                        } else {
                            return sdf.format(calendar.getTime()) + " " + sdf1.format(calendar.getTime());
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                        return "";
    ...
    public int getPastDayBackgroundColor() {
        return mPastBackgroundColor;
    }

    public void setPastDayBackgroundColor(int pastBackgroundColor) {
        mPastBackgroundColor = pastBackgroundColor;
        mPastBackgroundPaint.setColor(mPastBackgroundColor);
        invalidate();
    }

    public int getFutureDayBackgroundColor() {
        return mFutureBackgroundColor;
    }

    public void setFutureDayBackgroundColor(int futureDayBackgroundColor) {
        mFutureBackgroundColor = futureDayBackgroundColor;
        mFutureBackgroundPaint.setColor(mFutureBackgroundColor);
        invalidate();
    }
    ...
    public void goToDate(Calendar date) {
        ...
        date.set(Calendar.HOUR_OF_DAY, date.get(Calendar.HOUR_OF_DAY));
        date.set(Calendar.MINUTE, date.get(Calendar.MINUTE));
        date.set(Calendar.SECOND, date.get(Calendar.SECOND));
        date.set(Calendar.MILLISECOND, date.get(Calendar.MILLISECOND));
        ...
        today.set(Calendar.MINUTE, today.get(Calendar.MINUTE));
        today.set(Calendar.SECOND, today.get(Calendar.SECOND));
        today.set(Calendar.MILLISECOND, today.get(Calendar.MILLISECOND));
